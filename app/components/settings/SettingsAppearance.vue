...
async function downloadFont() {
  ...
  try {
    ...
    await refresh()
    form.font = 'custom'
    toast.add({ title: t('settings.appearance.fontDownloaded', { family }), icon: 'i-lucide-check', color: 'success' })
  }
  catch (err) {
    const e = err as { data?: { statusMessage?: string } }
    toast.add({ title: e.data?.statusMessage ?? t('settings.appearance.fontDownloadFailed'), color: 'error' })
  }
  finally {
...
async function removeFont() {
  ...
  try {
    ...
    toast.add({ title: t('settings.appearance.fontRemoved'), color: 'success' })
  }
  catch {
    toast.add({ title: t('settings.appearance.fontRemoveFailed'), color: 'error' })
  }
  finally {
    fontBusy.value = false
  }
...
const tvItems = computed(() => [
  { label: t('settings.appearance.tvAuto'), value: 'auto', icon: 'i-lucide-sunrise' },
  { label: t('settings.appearance.tvLight'), value: 'light', icon: 'i-lucide-sun' },
  { label: t('settings.appearance.tvDark'), value: 'dark', icon: 'i-lucide-moon' },
] as const)
...
const modeItems = computed(() => [
  { label: t('settings.appearance.modeLight'), value: 'light', icon: 'i-lucide-sun' },
  { label: t('settings.appearance.modeDark'), value: 'dark', icon: 'i-lucide-moon' },
  { label: t('settings.appearance.modeAuto'), value: 'system', icon: 'i-lucide-monitor' },
])
...
async function save() {
  ...
  try {
    ...
    toast.add({ title: t('settings.appearance.saved'), color: 'success' })
  }
  catch {
    toast.add({ title: t('settings.appearance.saveFailed'), color: 'error' })
  }
  finally {
    busy.value = false
  }
...
<template>
  <UCard>
    ...
    <div class="space-y-5">
      ...
      <UFormField :label="$t('settings.appearance.thisDevice')" :help="$t('settings.appearance.thisDeviceHelp')">
      ...
      <!-- Household-wide font -->
      <UFormField :label="$t('settings.appearance.font')" :help="$t('settings.appearance.fontHelp')">
      ...
      <UFormField
        v-if="isAdmin"
        :label="$t('settings.appearance.addGoogleFont')"
        :help="$t('settings.appearance.addGoogleFontHelp')"
      >
        <div class="flex flex-wrap items-center gap-2">
          <UInput
            v-model="customFamily"
            :placeholder="$t('settings.appearance.googleFontPlaceholder')"
          ...
          <UButton
            ...
            @click="removeFont"
          >
            {{ $t('settings.appearance.removeFont', { family: customFont.family }) }}
          </UButton>
        </div>
      </UFormField>
      ...
      <!-- Wall display theme -->
      <UFormField
        :label="$t('settings.appearance.tvTheme')"
        :help="tvTheme === 'auto' && !hasLocation
          ? $t('settings.appearance.tvThemeNoLocation')
          : $t('settings.appearance.tvThemeHelp')"
      >
        <div class="flex flex-wrap gap-2">
          <UButton
            v-for="opt in tvItems"
            :key="opt.value"
            :icon="opt.icon"
            :label="opt.label"
            :variant="tvTheme === opt.value ? 'solid' : 'soft'"
            :color="tvTheme === opt.value ? 'primary' : 'neutral'"
            class="min-h-11"
            :disabled="!isAdmin"
            @click="tvTheme = opt.value"
          />
        </div>
      </UFormField>

      <!-- Accent per mode -->
      <UFormField :label="$t('settings.appearance.accentLight')">
        <div class="flex flex-wrap gap-2">
          <button
      ...
      <UFormField :label="$t('settings.appearance.accentDark')">
        <div class="flex flex-wrap gap-2">
          <button
            v-for="c in ACCENT_COLORS"
            :key="`d-${c}`"
      ...
      <p v-else class="text-sm text-slate-500 dark:text-slate-400">
        {{ $t('settings.appearance.adminOnly') }}
      </p>
    </div>
  </UCard>
</template>
